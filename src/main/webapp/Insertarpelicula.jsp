...
<body>
	...
	<div id="crud" align="center">
		<form action="controllerCRUDPeliculas" method="post"
			...
			<input type="text" id="nombre" name="nombre"> <br> <label>PORTADA</label> <br>
			<label for="imagenp" class="subir">
    <i class="fas fa-cloud-upload-alt"></i> Subir imagen
</label>
<input id="imagenp"  type="file"  name="imagenp" accept="image/png,.jpg"  type="file" onclick="fondonegroimagenselecionada()" style='display: none;'/>
			 <br> <br> <label>FECHA
				DE ESTRENO</label> <br> <input type="date" id="fecha" name="fecha"
				value="2019-08-10" /> <br> <label>DURACION</label> <br> <label>HORAS</label>
			<label> MINUTOS</label> <br> <select id="hora" name="hora">
			...
			<input type="submit" id="Confirmar" value="GUARDAR ">
	...
	<script type="text/javascript">
		...
		function CargarCombobox() {
			...
			if (id != "0") {
				...
				var fecha = datos.get('year').replace(".", "").replace('ene',
						'Jan').replace('abr', 'Apr').replace('ago', 'Aug');
				document.getElementById('fecha').value = formatofecha(fecha);
				document.getElementById('sinopsis').value = datos
						.get('sinopsis');
				...
				document.getElementById('portadaa').value = datos
						.get('portada');
			} else {
		...
		//Cuando el input cambie (se cargue un nuevo archivo) se va a ejecutar de nuevo el cambio de imagen y se verï¿½ reflejado.
